...
/*
 * ANDROID_KABI_REPLACE(_oldtype, _oldname, _new)
 *   Replace a field with a compatible new field.  Brought in with the
 *   android16-6.12 scheduler checkout (kernel/sched/sched.h uses it for
 *   the cfs_rq KMI field renames); defined in terms of this tree's
 *   existing _ANDROID_KABI_REPLACE worker so genksyms still sees the
 *   original field.
 */
